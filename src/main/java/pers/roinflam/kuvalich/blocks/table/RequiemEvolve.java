...
public class RequiemEvolve extends Block implements IHasModel, IHasItem {

    @Nullable
    private final ItemBlock itemBlock;

    public RequiemEvolve(@Nonnull String name, @Nonnull Material material, @Nonnull CreativeTabs creativeTabs) {
        super(material);
    ...
    public boolean onBlockActivated(@Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull IBlockState state, @Nonnull EntityPlayer playerIn, @Nonnull EnumHand hand, @Nonnull EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            ...
            playerIn.openGui(Reference.MOD_ID, KuvaLichGuiHandler.REQUIEM_EVOLVE, worldIn, x, y, z);
        }
        return true;
    }
