...
public class ReaperPrime extends KuvaWeaponBase {
    ...
    public ReaperPrime(@NotNull String name) {
        super(name);
    }
    ...
    public ItemStack getBaseAttribute(ItemStack itemStack) {
        ...
        double criticalStrikeProbability = 35 / 100.0;
        double criticalStrikeMultiplier = 25 / 10.0;
        double triggerChance = 25 / 100.0;
            ...
            if (damageSource.getImmediateSource() instanceof EntityLivingBase) {
                ...
                if (!attacker.getHeldItem(attacker.getActiveHand()).isEmpty()) {
                    ...
                    if (itemStack.getItem() instanceof ReaperPrime) {
                        ...
                        attacker.heal(KuvaWeapon.getMagnification(itemStack, evt.getAmount() * 0.1f));
                    }
                }
            }
        }
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
        if (entityLiving instanceof EntityPlayer) {
            ...
            if ((getMaxItemUseDuration(stack) - timeLeft) * 2 >= 10) {
                double speed = KuvaWeapon.getMagnification(stack, entityplayer.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue(), 3);

                double magnification = MathHelper.clamp(0.5, ((getMaxItemUseDuration(stack) - timeLeft) * 2 - 10) / 20.0,
                        1.5);

                entityplayer.addVelocity(entityplayer.getLookVec().x * speed * 4.25 * 5 * magnification, 0, entityplayer.getLookVec().z * speed * 4.25 * 5 * magnification);
                entityplayer.getCooldownTracker().setCooldown(stack.getItem(), (int) (60 * magnification));
    ...
    public double getAttackDamageAmount(@NotNull ItemStack itemStack) {
        return AttributesUtil.getDamage(KuvaWeapon.getMagnification(itemStack, ConfigKuvaWeapon.attackDamageReaperPrime));
    }

    @Override
    public double getAttackSpeedAmount(@NotNull ItemStack itemStack) {
        return AttributesUtil.getDamageSpeed(KuvaWeapon.getMagnification(itemStack, ConfigKuvaWeapon.attackSpeedReaperPrime, 2));
    }

    @Override
    public double getMovementSpeedAmount(@NotNull ItemStack itemStack) {
        return Math.max(0, KuvaWeapon.getMagnification(itemStack, ConfigKuvaWeapon.movementSpeedReaperPrime, 2));
    }
